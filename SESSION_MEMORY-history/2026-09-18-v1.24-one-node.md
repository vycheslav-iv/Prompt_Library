...
- `tests/_smoke_prompt_library.mjs` — 57 фаз (в т.ч. id=-1 → назначение, subgraph-id,
  позиция после узлов вывода, broadcast→reload, bulk-бар, три режима)
...
- **Ловушка id (v1.24b, кусалась на живой):** в `onNodeCreated` `this.id` ещё
  `-1` (LiteGraph: `onNodeCreated` вызывается до `graph.add`). Свой id — только
  `st.ownId()` В МОМЕНТ события. Плюс subgraph: id приходит как `"5:12"` →
  сверять `display_node` и последний сегмент.
- **Обложка и позиция:** если нода стоит ПОСЛЕ узлов вывода — картинка прогона
  уже прошла; поэтому первая картинка кладётся в запас `st.runImages` и
  подхватывается при приходе `saved_id` (чистится на success/error).
- **Порядок сохранения в `execute`:** обложка подтягивается ПОСЛЕ прогона —
  не тащить её обратно проводом: это и было кольцо в графе. `_output_linked` —
  только авто-сквозь для старых графов, не менять без причины.
