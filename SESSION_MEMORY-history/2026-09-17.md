...
- **Папка не восстанавливалась**: save пишется (в файле `Fs/FAS` есть), restore
  опаздывал. Исследование ядра показало: `onConfigure(info)` получает данные
  воркфлоу — читаем `save_folder` синхронно из `info` (named + позиция `[2]`),
  retry `restoreFolder` удалён полностью (ноль polling). Плюс одноразовый
  reconcile после `reload()`. SPEC §23 переписан, скилл обновлён. НЕ коммичено —
  ждёт живого теста (рестарт + Ctrl+F5)
