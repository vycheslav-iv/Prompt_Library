...
- **Ловушка subgraph (v1.24):** в событии `executed` id ноды внутри subgraph
  приходит с префиксом (`"5:12"`) — сверять по `display_node` и последнему
  сегменту id (иначе `saved_id` не сойдётся и обложка потеряется).
